...
public class Instrumentor {
	...
	public Instrumentor(String input){
		this.input = input;
		this.startJSCoverage();
	}

	
	public Instrumentor(String input, String instrumentMode){
		this.input = input;
		this.startJSCoverage();
	}
	
	public Instrumentor(String input, String instrumentMode, boolean devMode){
		this.input = input;
	...
	private void createFunctionList(String fileName){
		...
		try {
    		...
    		for(String line : lines){  
    			...
    			} else if(!IsCommentLine(line) && line.contains("dojo.declare") && !line.contains("_yuitest") && !line.contains("_jstestcoverage")){
    				moduleName = line.substring(line.indexOf("(\"")+2, line.indexOf("\",")); 
    			}
    			if(line.contains("_jstestcoverage_func(") && !line.contains("\"(anonymous") && !line.contains("function")){
