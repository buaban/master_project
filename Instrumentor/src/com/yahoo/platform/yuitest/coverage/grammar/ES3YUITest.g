...
//	: lhs=conditionalExpression	( { isLeftHandSideAssign(lhs, isLhs) }? assignmentOperator assignmentExpression )?
	: lhs=conditionalExpression	( { false }? assignmentOperator assignmentExpression )?
	| leftHandSideExpression assignmentOperator assignmentExpression
	;
...
//	: lhs=conditionalExpressionNoIn	( { isLeftHandSideAssign(lhs, isLhs) }? assignmentOperator assignmentExpressionNoIn )?
	: lhs=conditionalExpressionNoIn	( { false }? assignmentOperator assignmentExpressionNoIn )?
...
@after { 
	$program::functions.add("\"" + $functionDeclaration::funcName + ":" + $start.getLine() + "\"");
  	...
  	//$functionDeclaration::paramList = $formalParameterList.params;
	//System.out.println($functionDeclaration::paramList);	
...
@init {
    ...
    String rawFuncName = $functionExpression::funcName;
    if(rawFuncName.contains(":")){
    	rawFuncName.replace(":","");
    	$functionExpression::funcName = rawFuncName;
    }
...
@after {
    ...
    if(!$functionDeclaration.isEmpty()){
 	...
 	: LPAREN ( name=Identifier {$formalParameterList::paramList += $name.getText();} ( COMMA name=Identifier {$formalParameterList::paramList += "," + $name.getText();} )* )? RPAREN
	;
...
@init {
	if($objectDeclaration.isEmpty()){
		$functionExpressionBodyWithoutBraces::objName = "";
	} else {
		$functionExpressionBodyWithoutBraces::objName = $objectDeclaration::objectName;
	}
}
@after { 
	...
	-> cover_func(src={$program::name}, code={$text}, object={$functionExpressionBodyWithoutBraces::objName}, name={$functionExpression::funcName}, line={$functionExpression::funcLine}, params={$functionExpressionBodyWithoutBraces::typesList})
	;
...
@after {
	...
	: sourceElement sourceElement* 
	{
		try {		  
		    if(!$functionDeclaration.isEmpty()){
				...
				$functionDeclarationBodyWithoutBraces::typesList = determineParam($functionDeclaration::paramList,$text,$functionDeclaration::funcName);
	...
	-> {$functionDeclaration::funcName!=null}? cover_func(src={$program::name}, code={$text}, object={$functionDeclarationBodyWithoutBraces::objName}, name={$functionDeclaration::funcName}, line={$functionDeclaration::funcLine},params={$functionDeclarationBodyWithoutBraces::typesList})
