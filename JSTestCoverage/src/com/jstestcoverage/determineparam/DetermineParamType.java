...
public class DetermineParamType {	
	...
	private String determineType(String param){		
		...
		DetermineTypeBase dm = new DetermineByMethod(this.funcBody,param);
		type = dm.execute();	
		if(type!=null){
			return type;
		} else {
			DetermineTypeBase dop = new DetermineByOperator(this.funcBody,param);
			type = dop.execute();	
			
			if(type!=null){
				return type;
			} else {
				DetermineTypeBase dp = new DetermineByParam(this.funcBody,param);
				type = dp.execute();	
				if(type!=null){
					return type;
				}
